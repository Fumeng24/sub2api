...
type UpdateSettingsRequest struct {
	...
	// Extension settings. Pointers preserve the stored value for official
	// clients that do not include these fields in their settings payload.
...
func applyBusinessSettingsUpdate(settings, previous *service.SystemSettings, req UpdateSettingsRequest) {
	settings.GroupRateDiscountSettings = previous.GroupRateDiscountSettings
	if req.GroupRateDiscountSettings != nil {
		settings.GroupRateDiscountSettings = dto.GroupRateDiscountSettingsToService(*req.GroupRateDiscountSettings)
	}

	settings.TicketSystemConfig = previous.TicketSystemConfig
	if req.TicketSystemConfig != nil {
		settings.TicketSystemConfig = *req.TicketSystemConfig
	}
}
...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	settings := &service.SystemSettings{
		...
		ChannelMonitorMode: func() string {
			if req.ChannelMonitorMode != nil {
				mode := strings.ToLower(strings.TrimSpace(*req.ChannelMonitorMode))
				if mode == service.ChannelMonitorModeV2 {
					return service.ChannelMonitorModeV2
				}
				return service.ChannelMonitorModeV1
			}
	...
	applyBusinessSettingsUpdate(settings, previousSettings, req)
