...
func grokImageRequestRequiresUnsupportedEditing(endpoint service.GrokMediaEndpoint, routingModel string, requestInfo service.GrokMediaRequestInfo) bool {
	if endpoint != service.GrokMediaEndpointImagesEdits &&
		(endpoint != service.GrokMediaEndpointImagesGenerations || !requestInfo.HasImageEditInput()) {
		return false
	}
	return !service.GrokImageModelSupportsEditing(routingModel)
}
...
func (h *OpenAIGatewayHandler) handleGrokMedia(c *gin.Context, endpoint service.GrokMediaEndpoint, requestID string) {
	...
	if grokImageRequestRequiresUnsupportedEditing(endpoint, routingModel, requestInfo) {
		h.errorResponse(c, http.StatusBadRequest, "invalid_request_error", "This Grok image model does not support image editing")
		return
	}
	...
	for {
		...
		if err != nil {
			...
			if lastFailoverErr != nil {
				h.handleGrokMediaFailoverExhausted(c, endpoint, lastFailoverErr, false)
			} else {
		...
		if err != nil {
			...
			if errors.As(err, &failoverErr) {
				...
				if c.Writer.Size() != writerSizeBeforeForward {
					h.handleGrokMediaFailoverExhausted(c, endpoint, failoverErr, true)
					return
				}
				if !failoverErr.ShouldRetryNextAccount() {
					h.handleGrokMediaFailoverExhausted(c, endpoint, failoverErr, false)
					return
				}
				if endpoint.IsVideoLookupRequest() {
					h.handleGrokMediaFailoverExhausted(c, endpoint, failoverErr, false)
...
func (h *OpenAIGatewayHandler) handleGrokMediaFailoverExhausted(
	c *gin.Context,
	endpoint service.GrokMediaEndpoint,
	failoverErr *service.UpstreamFailoverError,
	streamStarted bool,
) {
	// Cloudflare replaces the generic 502 response with an HTML error page. For
	// the image API's retryable upstream 503, retain the actionable status and
	// expose only a stable, sanitized OpenAI-compatible error body.
	if (endpoint == service.GrokMediaEndpointImagesGenerations || endpoint == service.GrokMediaEndpointImagesEdits) &&
		...
		!service.IsUpstreamModelUnsupportedError(failoverErr.StatusCode, failoverErr.ResponseBody) {
		...
		h.handleStreamingAwareError(c, http.StatusServiceUnavailable, "upstream_error", service.ClientFacingTemporaryUnavailableMessage(), streamStarted)
		return
	}
