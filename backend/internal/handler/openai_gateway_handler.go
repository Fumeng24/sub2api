...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	for {
		...
		forwardBody := mappedBodyForResponses(attemptChannelMapping.Mapped, attemptChannelMapping.MappedModel)
		...
		h.recordCyberPolicyIfMarked(c, apiKey, account, subscription, reqModel, err != nil, cyberBlockKeyHTTP, attemptChannelMapping.ToUsageFields(reqModel, ""), service.HashUsageRequestPayload(forwardBody))
			...
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &failoverErr) {
					...
					if isOpenAICompactContextWindowFailover(c, failoverErr) {
						...
						msg = openAIContextWindowClientMessage(reqModel, failoverErr.ResponseBody, msg)
						service.MarkOpsClientBusinessLimited(c, service.OpsClientBusinessLimitedReasonLocalFeatureGate)
		...
		if result != nil {
			...
			h.gatewayService.ReportOpenAIAccountScheduleResultForRequest(account.ID, reqModel, schedulerEndpoint, true, result.FirstTokenMs)
		} else {
			h.gatewayService.ReportOpenAIAccountScheduleResultForRequest(account.ID, reqModel, schedulerEndpoint, true, nil)
		}
...
func (h *OpenAIGatewayHandler) Messages(c *gin.Context) {
	...
	for {
		...
		if result != nil {
			h.gatewayService.ReportOpenAIAccountScheduleResultForRequest(account.ID, scheduleModel, schedulerEndpoint, true, result.FirstTokenMs)
		} else {
			h.gatewayService.ReportOpenAIAccountScheduleResultForRequest(account.ID, scheduleModel, schedulerEndpoint, true, nil)
		}
...
func (h *OpenAIGatewayHandler) submitUsageRecordTask(parent context.Context, task service.UsageRecordTask) {
	...
	if h.usageRecordWorkerPool != nil {
		...
		).Warn("openai.usage_record_task_sync_fallback_after_drop")
	}
...
func (h *OpenAIGatewayHandler) handleFailoverExhausted(c *gin.Context, failoverErr *service.UpstreamFailoverError, streamStarted bool) {
	...
	// 使用默认的错误映射
	clientErr := upstreamClientErrorForOpenAIFailoverStatus(statusCode)
	h.handleStreamingAwareError(c, clientErr.Status, clientErr.Type, clientErr.Message, streamStarted)
}
...
func (h *OpenAIGatewayHandler) handleFailoverExhaustedSimple(c *gin.Context, statusCode int, streamStarted bool) {
	clientErr := upstreamClientErrorForOpenAIFailoverStatus(statusCode)
	service.SetOpsUpstreamError(c, statusCode, clientErr.Message, "")
	h.handleStreamingAwareError(c, clientErr.Status, clientErr.Type, clientErr.Message, streamStarted)
}
...
func upstreamClientErrorForOpenAIFailoverStatus(statusCode int) UpstreamClientError {
