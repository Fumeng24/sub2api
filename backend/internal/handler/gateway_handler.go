...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	if platform == service.PlatformGemini {
		...
		for {
			...
			h.gatewayService.ReportAccountScheduleSuccessForRequest(c.Request.Context(), account.ID, reqModel, schedulerEndpoint, result.FirstTokenMs)
					...
					if !fallbackUsed && fallbackGroupID != nil && *fallbackGroupID > 0 {
						...
						if err := h.billingCacheService.CheckBillingEligibility(c.Request.Context(), fallbackAPIKey.User, fallbackAPIKey, fallbackGroup, nil, service.PlatformFromAPIKey(fallbackAPIKey)); err != nil {
							reqLog.Warn("gateway.fallback_group_billing_check_failed",
								billingEligibilityFailureFields(c.Request.Context(), h.billingCacheService, err, fallbackAPIKey, fallbackGroup, nil, service.PlatformFromAPIKey(fallbackAPIKey), parsedReq.Body.Bytes(), reqModel, GetInboundEndpoint(c))...)
							status, code, message, retryAfter := billingErrorDetails(err)
							if retryAfter > 0 {
								c.Header("Retry-After", strconv.Itoa(retryAfter))
							}
...
func billingErrorDetails(err error) (status int, code, message string, retryAfter int) {
	...
	if errors.Is(err, service.ErrInsufficientBalance) || pkgerrors.Reason(err) == "INSUFFICIENT_BALANCE" {
		msg := pkgerrors.Message(err)
		if msg == "" {
			msg = "insufficient balance"
		}
		return http.StatusPaymentRequired, "billing_error", msg, 0
	}
	msg := pkgerrors.Message(err)
	if msg == "" {
		logger.L().With(
...
func (h *GatewayHandler) submitUsageRecordTask(parent context.Context, task service.UsageRecordTask) {
	...
	if h.usageRecordWorkerPool != nil {
		...
		).Warn("gateway.usage_record_task_sync_fallback_after_drop")
	}
