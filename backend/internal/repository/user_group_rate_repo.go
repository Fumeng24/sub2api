...
func (r *userGroupRateRepository) GetDiscountsByUserID(ctx context.Context, userID int64) (map[int64]float64, error) {
	query := `SELECT group_id, discount_multiplier FROM user_group_rate_multipliers WHERE user_id = $1 AND discount_multiplier IS NOT NULL`
	rows, err := r.sql.QueryContext(ctx, query, userID)
	if err != nil {
		return nil, err
	}
...
// GetEffectiveByUserID 获取用户所有专属分组最终有效倍率。
...
// GetDiscountsByUserIDs 批量获取多个用户的专属分组 discount_multiplier（仅返回非 NULL 的条目）
...
// SyncUserGroupDiscounts 同步用户的分组专属 discount_multiplier。
//   - 传入空 map：清空该用户所有行的 discount_multiplier；若其他配置也为 NULL 则整行删除。
//   - 值为 nil：清空对应行的 discount_multiplier（保留 rate_multiplier/rpm_override）。
//   - 值非 nil：upsert discount_multiplier（保留已有 rate_multiplier/rpm_override）。
