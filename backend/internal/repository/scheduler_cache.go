...
// filterSchedulerAccountGroups returns lightweight AccountGroup entries
// (no nested Group object) so scheduler-side group membership checks work on
// metadata-only accounts without carrying heavy group payloads.
