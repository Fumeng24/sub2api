...
func (r *userRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, filters service.UserListFilters) ([]service.User, *pagination.PaginationResult, error) {
	...
	if filters.HasGroupRateConfig {
		// Keep the filter in the user query so pagination and total count use the
		// same EXISTS condition. A fixed rate changes pricing only when it differs
		// from the group's default. A relative discount changes pricing only when
		// it is not 1x and no fixed rate takes priority. RPM-only rows are excluded.
		q = q.Where(func(s *entsql.Selector) {
			s.Where(entsql.P(func(b *entsql.Builder) {
				b.WriteString("EXISTS (SELECT 1 FROM user_group_rate_multipliers AS ugr JOIN groups AS g ON g.id = ugr.group_id AND g.deleted_at IS NULL WHERE ugr.user_id = ")
				b.WriteString(s.C(dbuser.FieldID))
				b.WriteString(" AND ((ugr.rate_multiplier IS NOT NULL AND ugr.rate_multiplier IS DISTINCT FROM g.rate_multiplier) OR (ugr.rate_multiplier IS NULL AND ugr.discount_multiplier IS NOT NULL AND ugr.discount_multiplier IS DISTINCT FROM 1)))")
			}))
		})
	}
