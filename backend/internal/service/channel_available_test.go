...
func TestResolveAvailableGroupModelsListConfig_IntersectsGroupListWithAccounts(t *testing.T) {
	got := resolveAvailableGroupModelsListConfig(
		Group{
			Platform: PlatformAnthropic,
			ModelsListConfig: GroupModelsListConfig{
	...
	require.True(t, got.Enabled)
	require.Equal(t, []string{"claude-sonnet-4-6", "claude-haiku-4-5"}, got.Models)
}

func TestResolveAvailableGroupModelsListConfig_EmptyAccountListMeansNoVisibleModels(t *testing.T) {
	got := resolveAvailableGroupModelsListConfig(
		Group{
			Platform:         PlatformAnthropic,
