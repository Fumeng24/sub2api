...
func (s *adminServiceImpl) CreateGroup(ctx context.Context, input *CreateGroupInput) (*Group, error) {
	...
	group := &Group{
		...
		AutoSortConfig:                  input.AutoSortConfig,
	}
	applyCreateOpenAIGroupFieldsCustom(group, input)
	sanitizeGroupMessagesDispatchFields(group)
	if group.Platform != PlatformOpenAI {
		group.AllowLive = false
	}
	sanitizeGroupReasoningEffortPolicy(group)
	sanitizeCustomOpenAIGroupFields(group)
...
func (s *adminServiceImpl) UpdateGroup(ctx context.Context, id int64, input *UpdateGroupInput) (*Group, error) {
	...
	if input.AutoSortConfig != nil {
		group.AutoSortConfig = *input.AutoSortConfig
	}
	applyUpdateOpenAIGroupFieldsCustom(group, input)
	sanitizeGroupMessagesDispatchFields(group)
	if group.Platform != PlatformOpenAI {
		group.AllowLive = false
	}
