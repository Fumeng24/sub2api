...
const (
	...
	defaultLoginAgreementDate    = "2026-06-23"
	defaultTermsContentMD        = `# 服务条款
...
平台可根据业务、合规、上游规则或安全需要更新条款。条款更新后，可能要求你重新确认。你继续使用本服务，即视为接受更新后的条款。`
	defaultUsagePolicyContentMD = `# 使用政策
...
你对通过本服务提交的输入、上传的文件、调用的模型、生成的输出以及后续使用方式承担全部责任。你应确保相关行为符合法律法规、第三方平台规则和服务政策。`
	defaultSupportedRegionsContentMD = `# 支持的国家与地区
...
如果平台发现或有合理理由认为你的账号、支付、网络、身份、组织关系或使用行为不符合服务地区要求，平台有权限制、暂停或终止你的账号、API 密钥、余额使用和相关服务。`
	defaultServiceSpecificTermsContentMD = `# 服务特定条款
...
func defaultLoginAgreementDocuments() []LoginAgreementDocument {
	return []LoginAgreementDocument{
		...
		},
		{
			...
			Title:     "服务特定条款",
			ContentMD: defaultServiceSpecificTermsContentMD,
		},
	}
}

func defaultLoginAgreementDocumentContent(id, title string) string {
	id = normalizeLoginAgreementDocumentID(id)
	title = strings.TrimSpace(title)
	switch {
	case id == "terms" || title == "服务条款":
		return defaultTermsContentMD
	case id == "usage-policy" || title == "使用政策":
		return defaultUsagePolicyContentMD
	case id == "supported-regions" || title == "支持的国家与地区" || title == "支持的国家和地区":
		return defaultSupportedRegionsContentMD
	case id == "service-specific-terms" || title == "服务特定条款":
		return defaultServiceSpecificTermsContentMD
	default:
		return ""
	}
}
...
func normalizeLoginAgreementDocuments(docs []LoginAgreementDocument) []LoginAgreementDocument {
	...
	for i, doc := range docs {
		...
		if content == "" {
			content = defaultLoginAgreementDocumentContent(doc.ID, title)
		}
		if title == "" && content == "" {
			continue
		}
