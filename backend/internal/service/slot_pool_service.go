...
func (s *slotPoolService) AcquireFromPool(
	...
	for i := 0; i < maxRetries; i++ {
		...
		if !ok {
			...
			if cachedAccount, err := s.schedulerCache.GetAccount(ctx, accountID); err == nil && cachedAccount != nil && cachedAccount.IsSchedulable() {
				// 验证账号仍属于当前 bucket
				if s.accountBelongsToBucket(cachedAccount, bucket) {
					// 检查容量后再决定是否放回
					currentLoad, _ := s.concurrencyCache.GetAccountConcurrency(ctx, accountID)
					if hasAvailableSlot(currentLoad, cachedAccount.Concurrency) {
						deferredPutBacks = append(deferredPutBacks, SlotPoolEntry{
							AccountID: accountID,
							Priority:  cachedAccount.Priority,
							LastUsed:  s.getLastUsed(cachedAccount),
						})
					}
				}
			}
...
func (s *slotPoolService) OnSlotReleased(ctx context.Context, accountID int64) error {
	...
	// 获取当前负载
	currentLoad, err := s.concurrencyCache.GetAccountConcurrency(ctx, accountID)
	if err != nil {
		return err
	}
	...
	if hasAvailableSlot(currentLoad, account.Concurrency) {
		...
		for _, bucket := range buckets {
			_ = s.cache.Add(ctx, bucket, accountID, account.Priority, now)
		}
	}
...
func (s *slotPoolService) RebuildBucketPool(ctx context.Context, bucket SchedulerBucket, accounts []*Account) error {
	...
	for _, acc := range accounts {
		...
		currentLoad, _ := s.concurrencyCache.GetAccountConcurrency(ctx, acc.ID)
		if hasAvailableSlot(currentLoad, acc.Concurrency) {
			entries = append(entries, SlotPoolEntry{
				AccountID: acc.ID,
				Priority:  acc.Priority,
				LastUsed:  s.getLastUsed(acc),
			})
		}
	}
