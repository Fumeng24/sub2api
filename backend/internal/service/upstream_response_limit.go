...
func anthropicTooLargeError(c *gin.Context) {
	writeClientClaudeError(c, http.StatusBadGateway, "upstream_error", "Upstream response too large")
}

// openAITooLargeError 以 OpenAI / Gemini 格式写入超限错误。
func openAITooLargeError(c *gin.Context) {
	writeClientOpenAIError(c, http.StatusBadGateway, "upstream_error", "Upstream response too large")
}
