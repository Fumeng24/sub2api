...
func (s *AccountTestService) testOpenAICompactConnection(c *gin.Context, account *Account, testModelID string) error {
	...
	req = req.WithContext(WithHTTPUpstreamProfile(req.Context(), HTTPUpstreamProfileOpenAINoHeaderTimeout))
...
// RunTestBackgroundWithOptions executes an account test in-memory with the same
// path as the interactive SSE test, but returns a structured result for jobs.
