	...
	if s.schedulerSnapshot != nil {
		...
		if err == nil {
			slog.Debug("account_scheduling_list_snapshot",
				...
				"use_mixed", useMixed,
				"count", len(accounts))
			// 仅在 debug 级别真正开启时才遍历账号并逐个打日志。
			// 否则 O(N) 的 range + slog.Debug(...) 即便被 slog 内部丢弃，
			// 每迭代仍有方法调用 + ...any 装箱开销（N=5000 时每请求 ~25k 次装箱）。
...
func (s *GatewayService) gatewayWeakFallbackEnabled() bool {
	if s == nil || s.cfg == nil {
		return false
	}
	return s.cfg.Gateway.Scheduling.WeakFallbackEnabled
}
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput, opts *recordUsageOpts) error {
	...
	// 获取费率倍数（优先级：用户专属 > 分组默认 > 系统默认），并叠加分组周期折扣。
