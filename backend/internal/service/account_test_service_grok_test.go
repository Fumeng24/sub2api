...
func TestAccountTestService_TestAccountConnection_GrokImagineUsesImagesGeneration(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	account := &Account{
		...
		Credentials: map[string]any{
			...
			"model_mapping": map[string]any{
				"grok-imagine-image": "vendor-grok-image",
			},
		},
	}
	repo := &mockAccountRepoForGemini{accountsByID: map[int64]*Account{account.ID: account}}
	...
	err := svc.TestAccountConnection(c, account.ID, "grok-imagine-image", "draw a cat", AccountTestModeDefault)
	require.NoError(t, err)
	...
	require.Equal(t, "vendor-grok-image", gjson.GetBytes(upstream.lastBody, "model").String())
	require.Equal(t, "draw a cat", gjson.GetBytes(upstream.lastBody, "prompt").String())
	require.Equal(t, int64(1), gjson.GetBytes(upstream.lastBody, "n").Int())
