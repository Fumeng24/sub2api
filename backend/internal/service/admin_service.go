...
func (s *adminServiceImpl) GetGroupModelsListCandidates(ctx context.Context, id int64, platform string) ([]string, error) {
	...
	return groupModelsListCandidatesFromAccounts(platform, accounts), nil
}

func groupModelsListCandidatesFromAccounts(platform string, accounts []Account) []string {
	normalizedPlatform := strings.ToLower(strings.TrimSpace(platform))
	seen := make(map[string]struct{})
	candidates := make([]string, 0)
	for _, acc := range accounts {
		if normalizedPlatform != "" && strings.ToLower(strings.TrimSpace(acc.Platform)) != normalizedPlatform {
			continue
		}
...
func (s *adminServiceImpl) PreviewGroupRateChangeNotification(ctx context.Context, groupID int64, input GroupRateChangeNotificationInput) (*GroupRateChangeNotificationPreview, error) {
	...
	recentUsers, err := s.usageLogRepo.ListRecentGroupUsers(ctx, groupID, since, groupRateChangeNoticeMaxUsers)
	if err != nil {
		return nil, err
	}
