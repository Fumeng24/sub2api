...
func hasOpenAIExplicitImageGenerationTool(reqBody map[string]any) bool {
	...
	for _, rawTool := range tools {
		...
		if strings.TrimSpace(firstNonEmptyString(toolMap["type"])) == "image_generation" && openAIMapImageGenerationToolIsExplicit(toolMap) {
			return true
		}
	}
	return false
...
func removeOpenAIImplicitImageGenerationTools(reqBody map[string]any) bool {
	...
	for _, rawTool := range tools {
		toolMap, ok := rawTool.(map[string]any)
		if ok && strings.TrimSpace(firstNonEmptyString(toolMap["type"])) == "image_generation" && !openAIMapImageGenerationToolIsExplicit(toolMap) {
			modified = true
			continue
		}
