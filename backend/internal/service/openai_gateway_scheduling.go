...
// openAICompactSupportTier classifies an OpenAI-compatible account by the
// historical compact probe result.  The tier is a preference signal only:
// every schedulable OpenAI-compatible account is allowed to attempt compact
// requests because this gateway's upstream relays expose the same request
// surface for normal and compact traffic.  In particular, an old
// openai_compact_supported=false/force_off flag must not remove an otherwise
// usable account from the compact failover pool.
// 0 = not an OpenAI-compatible account, 1 = unknown or legacy negative probe,
// 2 = explicitly supported.
func openAICompactSupportTier(account *Account) int {
	...
	// A negative/forced-off probe is retained as a low-preference diagnostic
	// state, not a routing block.  The upstream account can still serve the
	// compact request and must remain available for failover.
	return 1
}
...
// ordinary scheduling gate so diagnostics can classify profit vetoes last.
