...
func (_q *TicketQuery) QueryReads() *TicketReadQuery {
	query := (&TicketReadClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *TicketQuery) WithReads(opts ...func(*TicketReadQuery)) *TicketQuery {
	query := (&TicketReadClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TicketQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Ticket, error) {
	...
	if query := _q.withReads; query != nil {
		if err := _q.loadReads(ctx, query, nodes,
			func(n *Ticket) { n.Edges.Reads = []*TicketRead{} },
			func(n *Ticket, e *TicketRead) { n.Edges.Reads = append(n.Edges.Reads, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TicketQuery) loadReads(ctx context.Context, query *TicketReadQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *TicketRead)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Ticket)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(ticketread.FieldTicketID)
	}
	query.Where(predicate.TicketRead(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(ticket.ReadsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
