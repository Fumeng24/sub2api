...
type UserSubscription struct {
	...
	// Enable auto-reset-with-cost when daily quota is exhausted
	AutoResetDaily bool `json:"auto_reset_daily,omitempty"`
...
func (*UserSubscription) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case usersubscription.FieldAutoResetDaily:
	...
	for i := range columns {
		...
		case usersubscription.FieldAutoResetDaily:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field auto_reset_daily", values[i])
			} else if value.Valid {
				_m.AutoResetDaily = value.Bool
			}
...
func (_m *UserSubscription) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("auto_reset_daily=")
	builder.WriteString(fmt.Sprintf("%v", _m.AutoResetDaily))
	builder.WriteString(", ")
