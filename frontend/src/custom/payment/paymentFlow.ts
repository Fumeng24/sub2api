...
export function getVisibleMethods(methods: Record<string, MethodLimit>): Record<string, MethodLimit> {
  ...
  Object.entries(methods).forEach(([type, limit]) => {
    ...
    const methodKey = normalized || type.trim().toLowerCase()
    if (!methodKey) return

    const isCanonical = type === normalized || type.trim().toLowerCase() === methodKey
    const existing = visible[methodKey]
    if (!existing || isCanonical) {
      visible[methodKey] = { ...limit }
    }
  })
