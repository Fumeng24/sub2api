...
export interface UpstreamSub2APIAccountStatus {
  ...
  probe_source?: 'api_key' | 'panel_login' | string
  proxy_used?: boolean
  status: string
  message?: string
  fetched_at: string
...
export async function updateSchedulerConfig(
  id: number,
  updates: UpdateSchedulerConfigRequest
): Promise<Account> {
  const { data } = await apiClient.put<Account>(`/admin/accounts/${id}/scheduler-config`, updates)
  return data
}
