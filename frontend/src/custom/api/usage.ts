...
export interface UsageDashboardBreakdownParams {
  start_date?: string
  end_date?: string
  api_key_id?: number
  group_id?: number
...
export async function getStatsByDateRange(
  ...
  endDate: string,
  apiKeyId?: number,
  filters?: Pick<UsageDashboardBreakdownParams, 'model' | 'group_id' | 'request_type' | 'stream' | 'billing_type' | 'billing_mode'>
): Promise<UsageStatsResponse> {
 ...
 */
export async function getDashboardModels(params?: UsageDashboardBreakdownParams): Promise<ModelStatsResponse> {
  const { data } = await apiClient.get<ModelStatsResponse>('/usage/dashboard/models', { params })
  return data
}

/**
 * Get user group usage statistics
 * @param params - Query parameters for filtering
 * @returns Group usage statistics for current user
 */
export async function getDashboardGroups(params?: UsageDashboardBreakdownParams): Promise<GroupStatsResponse> {
  const { data } = await apiClient.get<GroupStatsResponse>('/usage/dashboard/groups', { params })
  return data
}

/**
 * Get user inbound endpoint usage statistics
 * @param params - Query parameters for filtering
 * @returns Endpoint usage statistics for current user
 */
export async function getDashboardEndpoints(params?: UsageDashboardBreakdownParams): Promise<EndpointStatsResponse> {
  const { data } = await apiClient.get<EndpointStatsResponse>('/usage/dashboard/endpoints', { params })
  return data
}
