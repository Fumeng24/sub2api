...
import UsageView from '@/custom/user/WegooUsageView.vue'
...
const messages: Record<string, string> = {
  ...
  'usage.inputCost': 'Input Cost',
  'usage.outputCost': 'Output Cost',
  'usage.cacheCreationCost': 'Cache Creation Cost',
  'usage.cacheReadCost': 'Cache Read Cost',
  ...
  'usage.billed': 'Billed',
  'usage.allApiKeys': 'All Credentials',
  'usage.allModels': 'All Models',
  'usage.allTypes': 'All Types',
  'usage.apiKeyFilter': 'Access Credential',
  'usage.model': 'Model',
  ...
  'usage.exportHeaders.time': 'Time',
  'usage.exportHeaders.credentialName': 'Access Credential Name',
  'usage.exportHeaders.group': 'Group',
  'usage.exportHeaders.model': 'Model',
  'usage.exportHeaders.reasoningEffort': 'Reasoning Effort',
  'usage.exportHeaders.inboundEndpoint': 'Inbound Endpoint',
  'usage.exportHeaders.type': 'Type',
  'usage.exportHeaders.billingMode': 'Billing Mode',
  'usage.exportHeaders.inputTokens': 'Input Tokens',
  'usage.exportHeaders.outputTokens': 'Output Tokens',
  'usage.exportHeaders.cacheReadTokens': 'Cache Read Tokens',
  'usage.exportHeaders.cacheCreationTokens': 'Cache Write Tokens',
  'usage.exportHeaders.totalTokens': 'Total Tokens',
  'usage.exportHeaders.rateMultiplier': 'Rate Multiplier',
  'usage.exportHeaders.billedCost': 'Billed Cost ({currency})',
  'usage.exportHeaders.originalCost': 'Standard Cost ({currency})',
  'usage.exportHeaders.firstTokenMs': 'First Token (ms)',
  'usage.exportHeaders.durationMs': 'Duration (ms)',
}
...
const showFirstCostTooltip = async (wrapper: ReturnType<typeof mountUsageView>) => {
  await flushPromises()
  await nextTick()
  const trigger = wrapper.get('[data-testid="usage-cost-tooltip-trigger"]')
  await trigger.trigger('mouseenter', { clientX: 120, clientY: 80 })
  await nextTick()
}
...
describe('user UsageView tooltip', () => {
  beforeEach(() => {
    ...
    getDashboardModels.mockResolvedValue({ models: [], start_date: '2026-03-08', end_date: '2026-03-08' })
    ...
    getDashboardGroups.mockResolvedValue({ groups: [], start_date: '2026-03-08', end_date: '2026-03-08' })
    getDashboardEndpoints.mockResolvedValue({ endpoints: [], start_date: '2026-03-08', end_date: '2026-03-08' })
  ...
  it('shows fast service tier and unit prices in user tooltip', async () => {
    ...
    await showFirstCostTooltip(wrapper)
  ...
  it('exports csv with input and output unit price columns', async () => {
    ...
    expect(csvContent.slice(1)).toBe([
      'Time,API Key Name,Requested Model,Upstream Model,Reasoning Effort,Inbound Endpoint,IP Address,Type,Billing Mode,Input Tokens,Output Tokens,Cache Read Tokens,Cache Creation Tokens,Rate Multiplier,Billed Cost (CNY),Original Cost (CNY),First Token (ms),Duration (ms)',
      '2026-03-08T00:00:00Z,demo-key,gpt-5.4,gpt-5.4-upstream,"\'-",,203.0.113.10,Sync,Token,4057,101,278272,4,0.0,¥0.63160440,¥0.63160440,12,345',
    ].join('\n'))
