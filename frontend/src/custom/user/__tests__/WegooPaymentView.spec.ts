...
import PaymentView from '@/custom/user/WegooPaymentView.vue'
import AmountInput from '@/custom/payment/WegooAmountInput.vue'
import { PAYMENT_RECOVERY_STORAGE_KEY } from '@/custom/payment/paymentFlow'
...
const fetchSubscriptionCapability = vi.hoisted(() => vi.fn().mockResolvedValue(false))
const showError = vi.hoisted(() => vi.fn())
const showInfo = vi.hoisted(() => vi.fn())
const showWarning = vi.hoisted(() => vi.fn())
const fetchCheckoutInfo = vi.hoisted(() => vi.fn())
...
vi.mock('@/custom/stores/paymentCheckout', () => ({
...
function checkoutInfoWithPlansFixture(options: {
  checkout?: Partial<ReturnType<typeof checkoutInfoFixture>>
  method?: Record<string, unknown>
  plan?: Record<string, unknown>
} = {}) {
  const base = checkoutInfoFixture()
  return {
    ...base,
    ...options.checkout,
    methods: {
...
function checkoutInfoWithMinAmountFixture(minAmount: number) {
  return {
    ...checkoutInfoFixture(),
...
async function mountSubscriptionConfirm(options: Parameters<typeof checkoutInfoWithPlansFixture>[0] = {}) {
  ...
  fetchCheckoutInfo.mockReset().mockResolvedValue(checkoutInfoWithPlansFixture(options))
...
describe('PaymentView payment recovery', () => {
  beforeEach(() => {
    ...
    showInfo.mockReset()
    showWarning.mockReset()
    fetchCheckoutInfo.mockReset()
    bridgeInvoke.mockReset()
    window.localStorage.clear()
    ;(window as Window & { WeixinJSBridge?: { invoke: typeof bridgeInvoke } }).WeixinJSBridge = undefined
  })

  it('restores a custom visible payment method as selected', async () => {
    const base = checkoutInfoFixture()
    fetchCheckoutInfo.mockResolvedValue({
      ...base,
      methods: {
        ...base.methods,
        ldc: {
    ...
    const wrapper = shallowMount(PaymentView, {
      global: {
        stubs: {
          ...
          PaymentStatusPanel: {
            props: ['paymentType'],
            template: '<button data-test="payment-status" @click="$emit(\'done\')">{{ paymentType }}</button>',
          },
    ...
    expect((wrapper.vm as unknown as { selectedMethod: string }).selectedMethod).toBe('ldc')
  })
})
...
describe('PaymentView WeChat JSAPI flow', () => {
  beforeEach(() => {
    ...
    showInfo.mockReset()
    showWarning.mockReset()
    fetchCheckoutInfo.mockReset().mockResolvedValue(checkoutInfoFixture())
    bridgeInvoke.mockReset()
    window.localStorage.clear()
  ...
  it('uses recharge-only copy when no subscription plans are available', async () => {
    routeState.query = {}
    fetchCheckoutInfo.mockResolvedValue(checkoutInfoFixture())

    const wrapper = shallowMount(PaymentView, {
      global: {
        stubs: {
          AppLayout: { template: '<div><slot /></div>' },
          UserPageHero: { props: ['title'], template: '<section><h1>{{ title }}</h1><slot name="titleSuffix" /><slot name="below" /></section>' },
          AmountInput: { props: ['prefix'], template: '<div data-test="amount-input" />' },
          Teleport: true,
          Transition: false,
        },
      },
    ...
    expect(wrapper.text()).toContain('payment.rechargeTitle')
    expect(wrapper.text()).not.toContain('payment.title')
    expect(wrapper.text()).not.toContain('payment.gateway.description')
  })

  it('defaults to card code top-up even when online top-up is available', async () => {
    routeState.query = {}
    fetchCheckoutInfo.mockResolvedValue(checkoutInfoFixture())
  ...
  it('uses configured minimum amount for online top-up quick amounts and validation', async () => {
    routeState.query = {}
    fetchCheckoutInfo.mockResolvedValue(checkoutInfoWithMinAmountFixture(200))
    ...
    const wrapper = shallowMount(PaymentView, {
      global: {
        stubs: {
          ...
          UserPageHero: { props: ['title'], template: '<section><h1>{{ title }}</h1><slot name="titleSuffix" /><slot name="below" /></section>' },
          AmountInput: {
            props: ['modelValue', 'amounts', 'min', 'max', 'prefix'],
            emits: ['update:modelValue'],
            template: '<div data-test="amount-input-stub" />',
          },
          Teleport: true,
          Transition: false,
        },
    ...
    await wrapper.findAll('button').find(button => button.text().includes('payment.rechargeMethod.nativeTitle'))?.trigger('click')
    await flushPromises()

    const amountInput = wrapper.findComponent(AmountInput)
    expect(amountInput.exists()).toBe(true)
    expect(amountInput.props('amounts')).toEqual([200, 500, 1000, 2000, 5000])
    expect(amountInput.props('min')).toBe(200)

    amountInput.vm.$emit('update:modelValue', 199)
    await flushPromises()

    expect(wrapper.text()).toContain('payment.amountTooLowCardCode')
  })

  it('keeps subscription copy when subscription plans are available', async () => {
    routeState.query = {}
    fetchCheckoutInfo.mockResolvedValue(checkoutInfoWithPlansFixture())
    ...
    expect(wrapper.text()).toContain('payment.gateway.description')
  })
