      ...
      >
        ...
        <template #cell-actions="{ row }">
          <div class="flex flex-wrap items-center gap-1.5" @click.stop>
            <button
              type="button"
              class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-[var(--apple-muted)] transition-colors hover:bg-[var(--apple-hover)] hover:text-[var(--apple-text)]"
            ...
            <button
              type="button"
              class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-[var(--apple-blue)] transition-colors hover:bg-[var(--apple-hover)] hover:text-[var(--apple-blue-hover)]"
...
import UserErrorDetailModal from '@/custom/user/WegooUserErrorDetailModal.vue'
...
const emit = defineEmits<{
  ...
  (e: 'createTicket', v: UserErrorRequest): void
}>()
...
// → 结果(状态→消息)→ 时间 → UA(用量明细 UA 同在时间之后的尾部) → 操作
