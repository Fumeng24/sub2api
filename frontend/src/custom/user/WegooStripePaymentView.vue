<template>
  <component :is="isPopup ? 'div' : AppLayout" :class="isPopup ? 'min-h-screen bg-[var(--apple-bg)]' : ''">
    <div class="mx-auto max-w-lg space-y-6 py-8" :class="isPopup ? 'px-4' : ''">
      ...
      <div v-else-if="initError" class="rounded-lg border border-[color:var(--apple-border)] bg-[var(--apple-surface)] p-8 text-center shadow-sm">
        <div class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-lg border border-[color:color-mix(in_srgb,var(--apple-danger)_28%,var(--apple-border))] bg-[color-mix(in_srgb,var(--apple-danger)_10%,var(--apple-surface))]">
          <Icon name="exclamationCircle" size="xl" class="text-[var(--apple-danger)]" />
        </div>
        <h3 class="text-lg font-semibold text-[var(--apple-text)]">{{ t('payment.stripeLoadFailed') }}</h3>
        <p class="mt-2 text-sm text-[var(--apple-muted)]">{{ initError }}</p>
        <button class="btn btn-primary mt-6" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') }}</button>
      </div>
      <template v-else>
        <!-- 金额头部 -->
        <div v-if="order" class="rounded-lg border border-[color:var(--apple-border)] bg-[var(--apple-surface)] p-5 text-center shadow-sm">
          <p class="text-sm font-medium text-[var(--apple-muted)]">{{ t('payment.actualPay') }}</p>
          <p class="mt-1 text-3xl font-semibold text-[var(--apple-text)]">{{ formatGatewayAmount(order.pay_amount) }}</p>
          <p class="mt-2 text-xs text-[var(--apple-muted-2)]">{{ t('payment.orders.orderId') }} #{{ order.id }}</p>
        </div>
        ...
        <template v-if="wechatQrUrl">
          <div class="rounded-lg border border-[color:var(--apple-border)] bg-[var(--apple-surface)] p-6 shadow-sm">
            <div class="flex flex-col items-center space-y-4">
              <p class="text-lg font-semibold text-[var(--apple-text)]">{{ t('payment.qr.scanWxpay') }}</p>
              <div class="relative rounded-lg border border-[color:color-mix(in_srgb,var(--apple-success)_32%,var(--apple-border))] bg-white p-4">
                <img :src="wechatQrUrl" alt="WeChat Pay QR" class="h-56 w-56 rounded" />
                <div class="pointer-events-none absolute inset-0 flex items-center justify-center">
                  <span class="rounded-full bg-[#2BB741] p-2 ring-4 ring-white">
              ...
              <p class="text-center text-sm leading-6 text-[var(--apple-muted)]">{{ t('payment.qr.scanWxpayHint') }}</p>
            </div>
          </div>
          <div class="rounded-lg border border-[color:var(--apple-border)] bg-[var(--apple-surface)] p-4 text-center shadow-sm">
            <p class="text-sm text-[var(--apple-muted)]">{{ t('payment.qr.waitingPayment') }}</p>
          </div>
        </template>
        ...
        <template v-else-if="redirecting">
          <div class="rounded-lg border border-[color:var(--apple-border)] bg-[var(--apple-surface)] p-6 shadow-sm">
            <div class="flex flex-col items-center space-y-4 py-4">
              ...
              <p class="text-center text-sm leading-6 text-[var(--apple-muted)]">{{ t('payment.stripeRedirecting') }}</p>
            </div>
          </div>
        </template>
        ...
        <template v-else-if="stripeSuccess">
          <div class="rounded-lg border border-[color:var(--apple-border)] bg-[var(--apple-surface)] p-6 text-center shadow-sm">
            <div class="flex flex-col items-center gap-3 py-4">
              ...
              <p class="text-sm text-[var(--apple-muted)]">{{ t('payment.stripeSuccessProcessing') }}</p>
            </div>
          </div>
        </template>
        ...
        <template v-else-if="showPaymentElement">
          <div class="rounded-lg border border-[color:var(--apple-border)] bg-[var(--apple-surface)] p-6 shadow-sm">
            <p class="mb-4 text-sm leading-6 text-[var(--apple-muted)]">{{ t('payment.stripeSecureCaption') }}</p>
            <div id="stripe-payment-element" class="min-h-[200px]"></div>
            <p v-if="stripeError" class="mt-4 text-sm text-[var(--apple-danger)]">{{ stripeError }}</p>
            <button class="btn btn-primary mt-6 w-full py-3 text-base" :disabled="stripeSubmitting || !stripeReady" @click="handleGenericPay">
        ...
        <!-- 错误状态 -->
        <div v-if="stripeError && !showPaymentElement" class="rounded-lg border border-[color:color-mix(in_srgb,var(--apple-danger)_28%,var(--apple-border))] bg-[color-mix(in_srgb,var(--apple-danger)_10%,var(--apple-surface))] p-4">
          <p class="text-sm text-[var(--apple-danger)]">{{ stripeError }}</p>
...
import { PAYMENT_RECOVERY_STORAGE_KEY, readPaymentRecoverySnapshot } from '@/custom/payment/paymentFlow'
