<template>
  <div class="gateway-payment-result flex min-h-screen items-center justify-center bg-[var(--apple-bg)] px-4">
    <div class="w-full max-w-lg space-y-5">
      ...
      <template v-else>
        ...
        <div class="rounded-lg border border-[color:var(--apple-border)] bg-[var(--apple-surface)] p-6 text-center shadow-sm">
          ...
          <p v-if="hasOrderId(order)" class="mt-3 text-xs text-[var(--apple-muted-2)]">
        ...
        <div v-if="order" class="rounded-lg border border-[color:var(--apple-border)] bg-[var(--apple-surface)] p-5 shadow-sm">
          ...
          <div class="space-y-3 text-sm">
            ...
            <div v-if="hasAmountFields(order) && shouldShowCreditedBalance(order)" class="flex justify-between">
              <span class="text-[var(--apple-muted)]">{{ t('payment.orders.creditedBalance') }}</span>
              <span class="font-medium text-[var(--apple-text)]">{{ formatBalanceCreditAmount(order.amount, localeCode) }}</span>
            </div>
...
import type { PublicOrderVerifyResult } from '@/custom/api/payment'
import type { OrderStatus, PaymentOrder } from '@/types/payment'
import { formatPaymentAmount, normalizePaymentCurrency } from '@/components/payment/currency'
import { formatBalanceCreditAmount, shouldShowCreditedBalance } from '@/custom/payment/orderAmounts'
