<template>
  <BaseDialog
    ...
    @close="$emit('close')"
  >
    <div v-if="loading" class="py-8 text-center text-sm text-[var(--apple-muted)]">
      {{ t('common.loading') }}
    </div>
    <div v-else-if="!detail" class="py-8 text-center text-sm text-[var(--apple-muted)]">
      {{ t('channelStatus.detailLoadError') }}
    </div>
    <div v-else data-test="user-monitor-detail" class="table-container">
      <table class="table">
        <thead>
          <tr>
        ...
        <tbody>
          <tr
            ...
            :key="m.model"
          >
            <td class="font-medium">{{ m.model }}</td>
            <td>
              <span
