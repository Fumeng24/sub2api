    ...
    <div class="space-y-6">
      ...
      <div class="card p-4 sm:p-5">
        <div class="grid gap-4">
          <div v-if="activeTab === 'errors'" class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div class="min-w-0">
          ...
          <div v-else class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-6">
            <div class="min-w-0">
          ...
          <div class="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:items-center sm:justify-end sm:gap-3">
            <button type="button" @click="refreshData" :disabled="activeTab === 'errors' ? errorLoading : loading" class="btn btn-secondary justify-center whitespace-nowrap">
              {{ t('common.refresh') }}
            </button>
            <button type="button" @click="resetFilters" class="btn btn-secondary justify-center whitespace-nowrap">
              {{ t('common.reset') }}
            </button>
            <div class="relative" ref="columnDropdownRef">
              <button
                ...
                class="btn btn-secondary w-full justify-center whitespace-nowrap px-2 sm:w-auto md:px-3"
              ...
              <div
                ...
                class="absolute right-0 top-full z-50 mt-1 max-h-80 w-48 overflow-y-auto rounded-lg border border-[color:var(--apple-border)] bg-[var(--apple-surface)] py-1 shadow-lg"
              >
                <button
            ...
            <button v-if="activeTab !== 'errors'" type="button" @click="exportToCSV" :disabled="exporting" class="btn btn-primary justify-center whitespace-nowrap">
      ...
      <template v-if="activeTab === 'usage'">
        <UsageTable
          ...
          :format-cost="formatSettlementAmount"
          :format-token-price-per-million="formatSettlementTokenPricePerMillion"
...
import Select, { type SelectOption } from '@/custom/common/WegooSelect.vue'
...
import UsageTable from '@/custom/admin/usage/WegooUsageTable.vue'
import ModelDistributionChart from '@/custom/charts/WegooModelDistributionChart.vue'
import GroupDistributionChart from '@/custom/charts/WegooGroupDistributionChart.vue'
import EndpointDistributionChart from '@/custom/charts/WegooEndpointDistributionChart.vue'
...
import { BILLING_MODE_IMAGE, getBillingModeLabel } from '@/custom/utils/billingMode'
...
const { settlementCurrency, formatSettlementAmount } = useSettlementCurrency()
const shouldOpenErrorsTab = typeof window !== 'undefined'
  ? new URLSearchParams(window.location.search).get('tab') === 'errors'
  : false
...
onMounted(() => {
  ...
  refreshData()
  if (shouldOpenErrorsTab && errorViewEnabled.value) {
    switchToErrors()
  }
})
...
watch(errorViewEnabled, (enabled) => {
  if (shouldOpenErrorsTab && enabled && activeTab.value !== 'errors') {
    switchToErrors()
  }
})
</script>
