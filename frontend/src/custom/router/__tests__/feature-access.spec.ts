...
vi.mock('@/custom/stores/subscriptionCapability', () => ({
  useSubscriptionCapabilityStore: () => ({
    hasSubscriptionGroups: true,
    capabilityLoaded: true,
    fetchSubscriptionCapability: vi.fn(),
  }),
}))
...
vi.mock('@/router/setupRedirect', () => ({
  resolveCompletedSetupRedirectPath: vi.fn(() => '/dashboard'),
}))
...
vi.mock('@/custom/utils/seo', () => ({
  applyRouteSeo: vi.fn(),
}))

vi.mock('@/custom/utils/chunkRecovery', () => ({
...
describe('feature route guard', () => {
  beforeAll(async () => {
    await import('@/custom/router')
  })

  beforeEach(() => {
    authStore.isAuthenticated = true
    authStore.isAdmin = false
    authStore.isSupport = false
    authStore.canAccessTicketAdmin = false
