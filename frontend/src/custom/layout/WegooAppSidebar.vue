  ...
  >
    ...
    <div class="sidebar-header" :class="{ 'sidebar-header-collapsed': sidebarCollapsed }">
      ...
      <div class="sidebar-brand" :class="{ 'sidebar-brand-collapsed': sidebarCollapsed }" :aria-hidden="sidebarCollapsed ? 'true' : 'false'">
        <router-link
          :to="homePath"
          class="sidebar-brand-title text-lg font-semibold transition-colors hover:text-primary-600 dark:hover:text-primary-400"
          @click="handleMenuItemClick(homePath)"
        >
          {{ siteName }}
        </router-link>
    ...
    <nav ref="sidebarNavRef" class="sidebar-nav scrollbar-hide">
      ...
      <template v-if="canAccessTicketAdmin">
        ...
        <div v-if="showPersonalSectionForPrivilegedUser" class="sidebar-section">
          ...
          <template v-for="item in privilegedPersonalNavItems" :key="item.path">
            <a
              v-if="item.href"
              :href="item.href"
              target="_blank"
...
import { useAdminSettingsStore, useAnnouncementStore, useAppStore, useAuthStore, useOnboardingStore } from '@/stores'
...
const messageBadge = computed(() => announcementStore.unreadCount)
    ...
    h(
      ...
      [
        ...
        h('path', {
          'stroke-linecap': 'round',
          'stroke-linejoin': 'round',
          d: 'M8.25 13.5h7.5M8.25 16.5h5.25M8.25 10.5h2.25'
        })
      ]
    )
}
...
const flagUserSubscriptions = () => subscriptionCapabilityStore.hasSubscriptionGroups
const flagPurchase = () => flagPayment() && paymentCheckoutStore.canAccessPurchase
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/messages', label: t('nav.siteMessages'), icon: BellIcon, badge: messageBadge.value },
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/invoices', label: t('nav.invoiceManagement'), icon: InvoiceIcon },
...
function refreshPurchaseAvailability(force = false) {
  ...
  if (appStore.backendModeEnabled && !authStore.canAccessTicketAdmin) {
    paymentCheckoutStore.clear()
    return
  }
  paymentCheckoutStore.fetchCheckoutInfo(force).catch((error) => {
...
watch(
  ...
  () => refreshPurchaseAvailability(true),
  { immediate: true }
)
