<template>
  <AppLayout>
    <div class="admin-apple-page admin-table-page">
      ...
      <template #filters>
              ...
              <div class="relative" ref="accountToolsDropdownRef">
                ...
                >
                  <div class="max-h-[70vh] overflow-y-auto p-2">
                    ...
                    <button class="account-tools-menu-item" @click="openBulkVerify">
                      <span class="account-tools-menu-icon bg-indigo-50 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-300">
                        <Icon name="sync" size="sm" />
                      </span>
                      <span class="flex-1 text-left">{{ t('admin.accounts.bulkVerify.button') }}</span>
                    </button>
        ...
        <div
          ...
          class="mt-2 flex flex-col gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800 dark:border-amber-700/40 dark:bg-amber-900/20 dark:text-amber-200 sm:flex-row sm:items-center sm:justify-between"
        >
          <span>{{ t('admin.accounts.listPendingSyncHint') }}</span>
          <button
            class="btn btn-secondary w-full px-2 py-1 text-xs sm:w-auto"
      ...
      <template #table>
        <AccountBulkActionsBar
          ...
          @bulk-test-models="openBulkModelTest"
        />
        ...
        >
          ...
          <template #cell-rate_multiplier="{ row }">
            <span class="inline-flex items-center gap-1 text-sm font-mono text-gray-700 dark:text-gray-300">
              <span>{{ formatRateMultiplier(row.rate_multiplier ?? 1) }}x</span>
              <span
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                :data-account-action-menu="row.id"
                :title="t('common.more')"
              >
    ...
    <CreateAccountModal v-if="showCreate" :show="showCreate" :proxies="proxies" :groups="groups" @close="showCreate = false" @created="() => reload(true)" />
    ...
    <AccountBulkModelTestModal v-if="showBulkModelTest" :show="showBulkModelTest" :accounts="bulkModelTestAccounts" @close="closeBulkModelTest" />
    ...
    <AccountActionMenu :show="menu.show" :account="menu.acc" :position="menu.pos" @close="menu.show = false" @copy="handleCopyAccount" @test="handleTest" @stats="handleViewStats" @schedule="handleSchedule" @reauth="handleReAuth" @refresh-token="handleRefresh" @recover-state="handleRecoverState" @reset-quota="handleResetQuota" @set-privacy="handleSetPrivacy" @create-spark-shadow="handleCreateSparkShadow" />
    ...
    <AccountBulkVerifyModal v-if="showBulkVerify" :show="showBulkVerify" @close="showBulkVerify = false" @finished="handleBulkVerifyFinished" />
    </div>
  </AppLayout>
</template>
...
import { useTableLoader } from '@/custom/composables/useTableLoader'
...
import AccountTableActions from '@/custom/admin/account/WegooAccountTableActions.vue'
import AccountTableFilters from '@/custom/admin/account/WegooAccountTableFilters.vue'
import AccountBulkActionsBar from '@/custom/admin/account/WegooAccountBulkActionsBar.vue'
import AccountActionMenu from '@/custom/admin/account/WegooAccountActionMenu.vue'
...
import AccountUsageCell from '@/custom/admin/account/WegooAccountUsageCell.vue'
...
import { formatRateMultiplier } from '@/custom/utils/groupRateDiscount'
import { proxyExpiryBadgeClass, proxyExpiryLabelKey } from '@/utils/proxyExpiry'
import type { Account, AccountPlatform, AccountType, Proxy as AccountProxy, AdminGroup, WindowStats, ClaudeModel, AccountSchedulerGroupScore } from '@/types'

const { t } = useI18n()
const appStore = useAppStore()
const authStore = useAuthStore()

const CreateAccountModal = defineAsyncComponent(() => import('@/custom/admin/account/WegooCreateAccountModal.vue'))
const EditAccountModal = defineAsyncComponent(() => import('@/custom/admin/account/WegooEditAccountModal.vue'))
const BulkEditAccountModal = defineAsyncComponent(() => import('@/custom/admin/account/WegooBulkEditAccountModal.vue'))
const SyncFromCrsModal = defineAsyncComponent(() => import('@/custom/admin/account/WegooSyncFromCrsModal.vue'))
const TempUnschedStatusModal = defineAsyncComponent(() => import('@/custom/admin/account/WegooTempUnschedStatusModal.vue'))
const ImportDataModal = defineAsyncComponent(() => import('@/custom/admin/account/WegooImportDataModal.vue'))
const ReAuthAccountModal = defineAsyncComponent(() => import('@/custom/admin/account/WegooReAuthAccountModal.vue'))
const AccountTestModal = defineAsyncComponent(() => import('@/custom/admin/account/WegooAccountTestModal.vue'))
const AccountBulkModelTestModal = defineAsyncComponent(() => import('@/custom/admin/account/AccountBulkModelTestModal.vue'))
const AccountStatsModal = defineAsyncComponent(() => import('@/custom/admin/account/WegooAccountStatsModal.vue'))
const ScheduledTestsPanel = defineAsyncComponent(() => import('@/custom/admin/account/WegooScheduledTestsPanel.vue'))
...
const AccountBulkVerifyModal = defineAsyncComponent(() => import('@/custom/admin/account/AccountBulkVerifyModal.vue'))
...
const showBulkModelTest = ref(false)
...
const bulkModelTestAccounts = ref<Account[]>([])
const statsAcc = ref<Account | null>(null)
const showSchedulePanel = ref(false)
const scheduleAcc = ref<Account | null>(null)
const scheduleModelOptions = ref<SelectOption[]>([])
const fallbackScheduleModelsByPlatform: Record<AccountPlatform, string[]> = {
...
const buildScheduleModelOptions = (models: ClaudeModel[], platform: AccountPlatform): SelectOption[] => {
  const source = models.length > 0
    ? models
    : fallbackScheduleModelsByPlatform[platform].map((id) => ({
        id,
        display_name: id,
        type: 'model',
        created_at: ''
  ...
  for (const model of source) {
    ...
    seen.add(id)
    options.push({ value: id, label: model.display_name || id })
  }
...
const openBulkVerify = () => {
  closeAccountToolsDropdown()
  showBulkVerify.value = true
}
...
// Fresh Grok billing/quota snapshots are authoritative; imported credentials are fallbacks.
...
function upstreamProbeSourceLabel(source?: string): string {
  switch (source) {
    case 'api_key':
      return t('admin.accounts.upstreamSub2API.probeSources.apiKey')
    case 'panel_login':
      return t('admin.accounts.upstreamSub2API.probeSources.panelLogin')
    default:
      return '-'
  }
}

function hasLiveUpstreamRate(status: UpstreamSub2APIAccountStatus): boolean {
  return typeof status.upstream_group_effective_rate_multiplier === 'number' ||
    typeof status.upstream_group_default_rate_multiplier === 'number'
}

function upstreamKeyQuotaLabel(status: UpstreamSub2APIAccountStatus): string {
  if (status.usage_mode === 'unlimited') {
    return t('admin.accounts.upstreamSub2API.statusKeyUnlimited')
  }
  const remaining = formatUpstreamSub2APIMoney(status.key_remaining, status.balance_unit)
  return t('admin.accounts.upstreamSub2API.statusKeyRemaining', { balance: remaining })
}
...
function getUpstreamSub2APIStatusMeta(row: Account): { className: string; dotClass: string } | null {
  ...
  if (status.status === 'ok' && !(status.probe_source === 'api_key' && !hasLiveUpstreamRate(status))) {
    return {
...
function getUpstreamSub2APILabel(row: Account): string {
  ...
  if (status.probe_source === 'api_key' && !hasLiveUpstreamRate(status)) {
    return [upstreamKeyQuotaLabel(status), t('admin.accounts.upstreamSub2API.statusMetadataUnavailable')].join(' · ')
  }
  const rate = formatUpstreamSub2APIRate(status.upstream_group_effective_rate_multiplier)
  return t('admin.accounts.upstreamSub2API.statusOk', { rate })
}

function getUpstreamSub2APITitle(row: Account): string {
  const status = upstreamSub2APIStatusMap.value.get(row.id)
  if (!status) return ''
  const liveMetadata = hasLiveUpstreamRate(status)
  const lines = [
    ...
    status.probe_source
      ? `${t('admin.accounts.upstreamSub2API.probeSource')}: ${upstreamProbeSourceLabel(status.probe_source)}`
      : '',
  ...
  if (status.usage_mode === 'unlimited') {
    lines.push(t('admin.accounts.upstreamSub2API.statusKeyUnlimited'))
  }
  if (status.probe_source === 'api_key' && !liveMetadata) {
    lines.push(`${t('admin.accounts.upstreamSub2API.metadata')}: ${t('admin.accounts.upstreamSub2API.statusMetadataUnavailable')}`)
  }
  if (status.message) {
...
const openBulkModelTest = () => {
  const selected = accounts.value.filter(account => isSelected(account.id))
  if (selected.length === 0) {
    appStore.showError(t('admin.accounts.bulkModelTest.noSelectedAccounts'))
    return
  }
  bulkModelTestAccounts.value = selected
  showBulkModelTest.value = true
}
const closeBulkModelTest = () => {
  showBulkModelTest.value = false
  bulkModelTestAccounts.value = []
}
...
const handleSchedule = async (a: Account) => {
  ...
  try {
    ...
    scheduleModelOptions.value = buildScheduleModelOptions(models, a.platform)
  } catch {
    scheduleModelOptions.value = buildScheduleModelOptions([], a.platform)
  }
}
