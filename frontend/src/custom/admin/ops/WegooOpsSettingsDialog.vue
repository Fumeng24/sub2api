...
import type { OpsAlertRuntimeSettings, EmailNotificationConfig, AlertSeverity, OpsAdvancedSettings, OpsMetricThresholds } from '@/views/admin/ops/types'
...
<template>
  <BaseDialog :show="show" :title="t('admin.ops.settings.title')" width="extra-wide" @close="emit('close')">
    ...
    <div v-else-if="runtimeSettings && emailConfig && advancedSettings" class="space-y-6">
      ...
      <!-- 数据采集频率 -->
      <div class="admin-ops-surface p-4">
      ...
      <!-- 高级设置 -->
      <details class="admin-ops-surface">
