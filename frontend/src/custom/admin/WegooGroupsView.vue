        ...
        <div class="admin-toolbar">
          ...
          <div class="admin-toolbar-filters">
            ...
            <Select
              ...
              class="admin-toolbar-filter-wide"
              @change="loadGroups"
            />
            <Select
          ...
          <div class="admin-toolbar-actions">
            ...
            <button
              ...
              class="btn btn-primary admin-toolbar-primary"
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              >
                ...
                <span class="text-xs">
                  {{ duplicatingGroupIds.has(row.id) ? t("admin.groups.duplicating") : t("admin.groups.duplicate") }}
                </span>
              </button>
              <button
...
import GroupRateMultipliersModal from "@/custom/admin/groups/WegooGroupRateMultipliersModal.vue";
import GroupRPMOverridesModal from "@/components/admin/group/GroupRPMOverridesModal.vue";
import GroupCapacityBadge from "@/components/common/GroupCapacityBadge.vue";
import ReasoningEffortPolicyFields from "@/components/admin/group/ReasoningEffortPolicyFields.vue";
import WegooCompositeRoutesModal from "@/custom/admin/groups/WegooCompositeRoutesModal.vue";
...
} from "@/custom/admin/config/messagesDispatch";
import {
...
// 复制账号的源分组选项（创建时）- composite 分组可汇总各平台账号
const copyAccountsGroupOptions = computed(() => {
  const eligibleGroups = groups.value.filter(
    (group) =>
      canCopyAccountsFromGroup(createForm.platform, group.platform) &&
      (group.account_count || 0) > 0,
  );
  return eligibleGroups.map((group) => ({
    value: group.id,
    label: copyAccountsGroupLabel(group),
  }));
});

// 复制账号的源分组选项（编辑时）- composite 分组可汇总各平台账号，排除自身
const copyAccountsGroupOptionsForEdit = computed(() => {
  const currentId = editingGroup.value?.id;
  const eligibleGroups = groups.value.filter(
    (group) =>
      canCopyAccountsFromGroup(editForm.platform, group.platform) &&
      (group.account_count || 0) > 0 &&
      group.id !== currentId,
  );
  return eligibleGroups.map((group) => ({
...
const createForm = reactive({
  ...
  // Codex 网页搜索按次计费；null 使用后端默认价 0.01 USD/次
...
const handlePreviewRateChangeNotice = async () => {
  ...
  try {
    rateNoticePreview.value =
      await previewRateChangeNotification(
...
const handleSendRateChangeNotice = async () => {
  ...
  try {
    const result = await sendRateChangeNotification(
      editingGroup.value.id,
      buildRateNoticePayload(),
    );
...
const openCompositeRoutes = (group: AdminGroup) => {
  compositeRoutesGroup.value = group;
  showCompositeRoutesModal.value = true;
};

const closeCompositeRoutes = () => {
