  ...
  <AppLayout>
    ...
    <TablePageLayout>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-business_category="{ value }">
            <span class="badge badge-gray">
              {{ balanceBusinessCategoryLabel(value) }}
            </span>
          </template>
    ...
    <Teleport to="body">
      <div v-if="showGenerateDialog" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div
          class="admin-apple-modal-panel relative z-10 w-full max-w-md p-6"
        >
    ...
    <Teleport to="body">
      ...
      >
        ...
        <div
          class="admin-apple-modal-panel relative z-10 w-full max-w-lg p-6"
        >
          ...
          <form data-test="batch-update-form" class="space-y-4" @submit.prevent="handleBatchUpdate">
            ...
            <div class="space-y-2">
              <label class="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                <input
                  v-model="batchUpdateForm.update_business_category"
                  type="checkbox"
                  class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
                />
                {{ t('admin.redeem.batchFields.businessCategory') }}
              </label>
              <Select
                v-if="batchUpdateForm.update_business_category"
                v-model="batchUpdateForm.business_category"
                :options="redeemBusinessCategoryOptions"
              />
            </div>
    ...
    <Teleport to="body">
      <div v-if="showResultDialog" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="admin-apple-modal-panel relative z-10 w-full max-w-lg overflow-hidden">
          <!-- Header -->
          <div
...
const columns = computed<Column[]>(() => [
  ...
  { key: 'business_category', label: t('admin.redeem.columns.businessCategory') },
...
const batchUpdateForm = reactive({
  ...
  update_business_category: false,
  business_category: '' as BalanceBusinessCategory
})
...
const resetBatchUpdateForm = () => {
  ...
  batchUpdateForm.update_business_category = false
  batchUpdateForm.business_category = ''
}
...
const buildBatchUpdateFields = (): BatchUpdateRedeemCodeFields | null => {
  ...
  if (batchUpdateForm.update_business_category) {
    fields.business_category = batchUpdateForm.business_category
  }
...
const handleBatchUpdate = async () => {
  ...
  const hasSelectedFields =
    ...
    batchUpdateForm.update_business_category
