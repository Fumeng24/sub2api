  ...
  <BaseDialog :show="show" :title="operation === 'add' ? t('admin.users.deposit') : t('admin.users.withdraw')" width="narrow" @close="$emit('close')">
    <form v-if="user" id="balance-form" @submit.prevent="handleBalanceSubmit" class="space-y-5">
      ...
      <div>
        ...
        <Select v-model="form.business_category" :options="balanceBusinessCategoryOptions" />
      </div>
...
import type { AdminUser, BalanceBusinessCategory } from '@/types'
...
const submitting = ref(false); const form = reactive({ amount: 0, notes: '', business_category: defaultBalanceBusinessCategory(props.operation) as BalanceBusinessCategory })
watch(() => props.show, (v) => { if(v) { form.amount = 0; form.notes = ''; form.business_category = defaultBalanceBusinessCategory(props.operation) } })
watch(() => props.operation, (operation) => { form.business_category = defaultBalanceBusinessCategory(operation) })
...
const handleBalanceSubmit = async () => {
  ...
  try {
    await adminAPI.users.updateBalance(props.user.id, form.amount, props.operation, form.notes, form.business_category)
