...
import ImportDataModal from '@/custom/admin/account/WegooImportDataModal.vue'
...
describe('ImportDataModal', () => {
  beforeEach(async () => {
    ...
    const { adminAPI } = await import('@/custom/api/admin')
  ...
  it('merges multiple selected JSON files before importing', async () => {
    ...
    const resultPanel = wrapper.find('.space-y-2.border')
    expect(resultPanel.exists()).toBe(true)
    expect(resultPanel.classes()).toContain('rounded-lg')
    expect(resultPanel.classes()).not.toContain('rounded-xl')
  })
