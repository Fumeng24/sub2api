  ...
  >
    ...
    >
      ...
      <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <div>
          <label class="input-label">{{ t('admin.accounts.rateScale') }}</label>
          <input v-model.number="rateScale" type="number" min="0" step="0.0001" class="input" :placeholder="'1'" />
          <p class="input-hint">{{ t('admin.accounts.rateScaleHint') }}</p>
        </div>
        <div>
          <label class="input-label">{{ t('admin.accounts.manualRate') }}</label>
          <input v-model.number="manualRate" type="number" min="0" step="0.0001" class="input" :placeholder="t('admin.accounts.manualRatePlaceholder')" />
          <p class="input-hint">{{ t('admin.accounts.manualRateHint') }}</p>
        </div>
      </div>
      <div class="border-t border-gray-200 pt-4 dark:border-dark-600">
      ...
      <!-- OpenAI Codex namespace tool flattening (OAuth compatibility) -->
      <div
      ...
      >
        <div class="flex items-center justify-between gap-4">
          <div class="min-w-0">
            ...
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('admin.accounts.openai.planTypeDesc') }}</p>
          </div>
...
import ModelWhitelistSelector from '@/custom/admin/account/WegooModelWhitelistSelector.vue'
...
} from '@/custom/composables/useModelWhitelist'
...
// 调度页倍率换算（存 account.extra）：
//   - rateScale: 上游倍率换算比例（如上游一元十刀填 0.1），缺省 1。最终倍率 = 上游倍率 × rateScale。
//   - manualRate: 手动倍率，仅当上游取不到倍率时作为回退展示/排序值。
const rateScale = ref<number | null>(null)
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  {
    const ex = (newAccount.extra as Record<string, unknown> | undefined) || {}
    rateScale.value = typeof ex.rate_scale === 'number' ? ex.rate_scale : null
    manualRate.value = typeof ex.manual_rate === 'number' ? ex.manual_rate : null
  }
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // 倍率换算字段写入 extra（merge 现有 extra，避免覆盖 openai_compact_mode 等其它键）。
    {
      const mergedExtra: Record<string, unknown> = {
        ...((props.account.extra as Record<string, unknown> | undefined) || {}),
      }
      if (typeof rateScale.value === 'number' && Number.isFinite(rateScale.value)) {
        mergedExtra.rate_scale = rateScale.value
      } else {
        delete mergedExtra.rate_scale
      }
      if (typeof manualRate.value === 'number' && Number.isFinite(manualRate.value)) {
        mergedExtra.manual_rate = manualRate.value
      } else {
        delete mergedExtra.manual_rate
      }
    ...
    if (props.account.type === 'apikey') {
      ...
      if (editUpstreamPanelType.value !== 'auto' || editUpstreamSub2APIEmail.value.trim()) {
