<template>
  <div class="flex items-center gap-2" :title="schedulingTitle">
    ...
    <div v-if="account.error_message && !isBalanceInsufficient" class="group/error relative">
      <svg
...
const isBalanceInsufficient = computed(() => {
  const raw = (props.account.error_message || '').trim()
  if (!raw) return false
...
const statusClass = computed(() => {
  if (hasError.value || isBalanceInsufficient.value) {
    return 'badge-danger'
  }
...
const statusText = computed(() => {
  if (isBalanceInsufficient.value) {
    return t('admin.accounts.status.insufficientBalance')
  }
  if (hasError.value) {
