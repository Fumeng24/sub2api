...
import UsageProgressBar from '@/custom/admin/account/WegooUsageProgressBar.vue'
import AccountQuotaInfo from '@/custom/admin/account/WegooAccountQuotaInfo.vue'
...
const handleGrokProbed = (result: GrokQuotaProbeResult) => {
  ...
  const merged: AccountUsageInfo = {
    ...
    grok_last_quota_probe_at:
      result.billing?.fetched_at ?? snapshot?.last_probe_at ?? current.grok_last_quota_probe_at,
    grok_last_headers_seen_at:
      snapshot?.last_headers_seen_at ?? current.grok_last_headers_seen_at,
    grok_last_status_code:
      result.status_code ?? snapshot?.status_code ?? current.grok_last_status_code,
