...
import AccountsView from '../WegooAccountsView.vue'
...
const {
  ...
  getAllProxies: vi.fn(),
  getAllGroups: vi.fn(),
  getUpstreamSub2APIStatus: vi.fn(),
...
describe('admin AccountsView — 外审 F2:spark 影子创建接线', () => {
  beforeEach(() => {
    localStorage.clear()
    for (const fn of [listAccounts, listWithEtag, getBatchTodayStats, getAllProxies, getAllGroups, getUpstreamSub2APIStatus, createSparkShadow, showSuccess, showError]) {
      fn.mockReset()
    }
    listAccounts.mockResolvedValue({ items: [], total: 0, page: 1, page_size: 20, pages: 0 })
    listWithEtag.mockResolvedValue({ notModified: true, etag: null, data: null })
    getBatchTodayStats.mockResolvedValue({ stats: {} })
    getUpstreamSub2APIStatus.mockResolvedValue([])
