...
} from "@/custom/api/admin/settings";
import type {
...
import PaymentProviderList from "@/custom/payment/WegooPaymentProviderList.vue";
...
import BackupSettings from "@/custom/admin/WegooBackupView.vue";
import EmailTemplateEditor from "@/custom/admin/settings/WegooEmailTemplateEditor.vue";
...
import { normalizeVisibleMethod } from "@/custom/payment/paymentFlow";
import { normalizeDocsLinkValue } from "@/custom/utils/docsLink";
import {
...
} from "@/custom/loginAgreementDefaults";
import {
...
// Global upstream billing and Ollama Cloud usage settings.
...
const defaultLoginAgreementUpdatedAt = siteDefaultLoginAgreementUpdatedAt;

function defaultLoginAgreementDocuments(): LoginAgreementDocument[] {
  return applySiteLoginAgreementDefaults([
    {
      id: "terms",
...
type SettingsForm = Omit<
  ...
  | "affiliate_bind_bonus_amount"
  | "payment_balance_recharge_unlock_threshold"
  | "group_rate_discount_settings"
  | "ticket_system_config"
> & {
  affiliate_bind_bonus_amount: number;
  payment_balance_recharge_unlock_threshold: number;
...
const form = reactive<SettingsForm>({
  ...
  payment_balance_recharge_unlock_threshold: 0,
  ...
  fallback_model_openai: "gpt-5.4-mini",
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      payment_balance_recharge_unlock_threshold:
        Number(form.payment_balance_recharge_unlock_threshold) || 0,
...
async function loadOllamaCloudUsageSettings() {
  ...
  try {
    ...
    // Keep fail-safe disabled defaults when this optional setting cannot be loaded.
  } finally {
