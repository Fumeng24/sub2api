...
async function handleSave() {
  ...
  const invalidTargeting = anyOf.length > 50 || anyOf.some((group) => {
    const allOf = group?.all_of ?? []
    if (allOf.length === 0 || allOf.length > 50) return true
    return allOf.some((condition) => {
      if (condition.type === 'subscription') {
        return condition.operator !== 'in' || !condition.group_ids?.length
      }
      if (condition.type === 'user') {
        return condition.operator !== 'in' ||
          !condition.user_ids?.length ||
          condition.user_ids.length > 100
      }
      return condition.type !== 'balance' ||
        !balanceOperators.has(condition.operator) ||
        !Number.isFinite(condition.value)
    })
  })
  if (invalidTargeting) {
    appStore.showError(t('admin.announcements.form.invalidTargeting'))
    return
  }
