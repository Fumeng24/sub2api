<template>
  <div :class="flat ? '' : 'card overflow-hidden'">
    ...
    >
      <span v-if="pendingIpCount > 0" class="text-xs text-[var(--apple-muted)]">
        ...
        <template #cell-cost="{ row }">
          <div class="text-sm">
            <div class="flex items-center gap-1.5">
              <span class="font-medium text-green-600 dark:text-green-400">{{ displayCost(row.actual_cost, 6) }}</span>
              <span
  ...
  <Teleport to="body">
    ...
    >
      <div class="whitespace-nowrap rounded-lg border border-gray-700 bg-gray-900 px-3 py-2.5 text-xs text-white shadow-md dark:border-gray-600 dark:bg-gray-800">
        ...
        <div class="space-y-1.5">
          ...
          <div class="mb-2 border-b border-gray-700 pb-1.5">
            ...
            <div v-if="tooltipData && tooltipData.input_cost > 0" class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-white">{{ displayCost(tooltipData.input_cost, 6) }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.output_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('admin.usage.outputCost') }}</span>
              <span class="font-medium text-white">{{ displayCost(tooltipData.output_cost, 6) }}</span>
            </div>
            <div v-if="tooltipData && hasImageOutputCost(tooltipData)" class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-pink-300">{{ displayCost(tooltipData.image_output_cost, 6) }}</span>
            </div>
            <!-- Token billing: show unit prices per 1M tokens -->
            <template v-if="getDisplayBillingMode(tooltipData) !== BILLING_MODE_IMAGE">
              <div v-if="tooltipData && tooltipData.input_tokens > 0" class="flex items-center justify-between gap-4">
                <span class="text-gray-400">{{ t('usage.inputTokenPrice') }}</span>
                <span class="font-medium text-sky-300">{{ displayTokenPricePerMillion(tooltipData.input_cost, tooltipData.input_tokens) }} {{ t('usage.perMillionTokens') }}</span>
              </div>
            ...
            <template v-else-if="tooltipData && isImageUsage(tooltipData)">
              ...
              <div class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-sky-300">{{ displayCost(imageUnitPrice(tooltipData), 6) }}</span>
              </div>
              <div class="flex items-center justify-between gap-4">
                <span class="text-gray-400">{{ t('usage.imageTotalPrice') }}</span>
                <span class="font-medium text-white">{{ displayCost(tooltipData.total_cost, 6) }}</span>
              </div>
            </template>
            <div v-else class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('usage.unitPrice') }}</span>
              <span class="font-medium text-sky-300">{{ displayCost(tooltipData?.total_cost, 6) }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.cache_creation_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('admin.usage.cacheCreationCost') }}</span>
              <span class="font-medium text-white">{{ displayCost(tooltipData.cache_creation_cost, 6) }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.cache_read_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('admin.usage.cacheReadCost') }}</span>
              <span class="font-medium text-white">{{ displayCost(tooltipData.cache_read_cost, 6) }}</span>
            </div>
          </div>
          <!-- Rate and Summary -->
          ...
          <div class="flex items-center justify-between gap-6">
            ...
            <span class="font-semibold text-green-400">{{ displayCost(tooltipData?.actual_cost, 6) }}</span>
          </div>
...
import { formatCacheTokens, formatMultiplier } from '@/custom/utils/formatters'
import { isOpenAICacheReadOnlyUsage } from '@/custom/utils/cacheUsage'
import { formatTokenPricePerMillion as defaultFormatTokenPricePerMillion } from '@/utils/usagePricing'
...
interface Props {
  ...
  /** 嵌入统一卡片内使用：去掉自身卡片外观 */
  flat?: boolean
  formatCost?: (amount: number, fractionDigits?: number) => string
  formatTokenPricePerMillion?: (cost: number | null | undefined, tokens: number | null | undefined) => string
}
...
const displayCost = (amount: number | null | undefined, fractionDigits = 6) => {
  const value = typeof amount === 'number' && Number.isFinite(amount) ? amount : 0
  return props.formatCost ? props.formatCost(value, fractionDigits) : `$${value.toFixed(fractionDigits)}`
}

const displayTokenPricePerMillion = (
  cost: number | null | undefined,
  tokens: number | null | undefined
) => props.formatTokenPricePerMillion
  ? props.formatTokenPricePerMillion(cost, tokens)
  : defaultFormatTokenPricePerMillion(cost, tokens)
