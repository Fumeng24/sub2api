<template>
  <div data-testid="usage-stats-cards" class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <div class="min-w-0 flex-1">
        ...
        <p class="break-words text-xs text-gray-400">
          <template v-if="showAccountCost && totalAccountCost != null">
            <span data-testid="usage-account-cost" class="text-orange-500">{{ t('usage.accountCost') }} ${{ totalAccountCost.toFixed(4) }}</span>
            <span> · </span>
          </template>
          <span>
            {{ t('usage.standardCost') }}
            <span data-testid="usage-standard-cost" :class="{ 'line-through': strikeStandardCost }">${{ (stats?.total_cost || 0).toFixed(4) }}</span>
          </span>
        </p>
      </div>
    </div>
    <div class="card flex min-w-0 items-center gap-3 p-4">
      ...
      <div class="min-w-0"><p class="text-xs font-medium text-gray-500">{{ t('usage.avgDuration') }}</p><p class="text-xl font-bold">{{ formatDuration(stats?.average_duration_ms || 0) }}</p></div>
    </div>
  </div>
</template>
...
defineOptions({ name: 'UsageStatsCards' })
