<template>
  <div v-if="usesMobileCards" class="space-y-3">
    <template v-if="loading">
      <div v-for="i in 5" :key="i" class="data-table-mobile-card rounded-lg border p-4">
        <div class="space-y-3">
          <div v-for="column in dataColumns" :key="column.key" class="flex justify-between">
            <div class="h-4 w-20 animate-pulse rounded bg-gray-200 dark:bg-dark-700"></div>
            <div class="h-4 w-32 animate-pulse rounded bg-gray-200 dark:bg-dark-700"></div>
          </div>
          <div v-if="hasActionsColumn" class="data-table-mobile-actions border-t pt-3">
    ...
    <template v-else-if="!data || data.length === 0">
      <div class="data-table-mobile-card rounded-lg border p-12 text-center">
        <slot name="empty">
    ...
    <template v-else>
      <div
        ...
        class="data-table-mobile-card rounded-lg border p-4"
      ...
      <thead class="table-header">
        <tr>
          <th
            ...
            :class="[
              'sticky-header-cell py-3 text-left text-xs font-semibold text-gray-500 dark:text-dark-400',
          ...
          >
            ...
            >
              <div :class="['flex min-w-0 items-center gap-1', getHeaderContentAlignmentClass(column)]">
      ...
      <tbody class="table-body divide-y">
        ...
        <template v-else>
          ...
          <tr
            ...
            class="data-table-row transition-colors"
            :class="{ 'cursor-pointer': clickableRows }"
...
interface Props {
  ...
  /** Render the desktop table layout at every viewport width. */
...
const usesTableLayout = computed(() => props.forceDesktopLayout || isDesktopViewport.value)
const usesMobileCards = computed(() => !usesTableLayout.value)
...
const shouldVirtualize = computed(() =>
  usesTableLayout.value && (sortedData.value?.length ?? 0) > (props.virtualizeThreshold ?? 100)
)
...
.table-wrapper {
  ...
  --data-table-sticky-bg: var(--apple-surface-solid, var(--apple-surface));
  --data-table-sticky-header-bg: var(--apple-surface-elevated-solid, var(--apple-surface-elevated));
  --data-table-sticky-hover-bg: color-mix(in srgb, var(--apple-blue) 5%, var(--data-table-sticky-bg));
  position: relative;
  overflow-x: auto;
  overflow-y: auto;
  flex: 1;
...
.dark .data-table-mobile-card {
  background: color-mix(in srgb, var(--apple-surface) 92%, white 8%);
}
...
.table-wrapper .table-header {
  ...
  top: 0;
  z-index: 200;
  background: var(--data-table-sticky-header-bg);
}
...
.sticky-col-right {
  right: 0;
  background: var(--data-table-sticky-bg);
...
tbody tr:hover .sticky-col {
  background: var(--data-table-sticky-hover-bg);
}
...
.table-wrapper {
  ...
  scrollbar-color: color-mix(in srgb, var(--apple-muted) 34%, transparent) transparent;
}
...
.table-wrapper::-webkit-scrollbar-thumb {
  background-color: color-mix(in srgb, var(--apple-muted) 34%, transparent);
...
.table-wrapper::-webkit-scrollbar-thumb:hover {
  background-color: color-mix(in srgb, var(--apple-muted) 54%, transparent);
}

.dark .table-wrapper::-webkit-scrollbar-thumb {
  background-color: color-mix(in srgb, var(--apple-muted) 38%, transparent);
}

.dark .table-wrapper::-webkit-scrollbar-thumb:hover {
  background-color: color-mix(in srgb, var(--apple-muted) 58%, transparent);
}

@supports (-moz-appearance:none) {
  .table-wrapper {
    scrollbar-width: thin;
    scrollbar-color: color-mix(in srgb, var(--apple-muted) 42%, transparent) transparent;
  }

  .dark .table-wrapper {
    scrollbar-color: color-mix(in srgb, var(--apple-muted) 48%, transparent) transparent;
  }
}
</style>
