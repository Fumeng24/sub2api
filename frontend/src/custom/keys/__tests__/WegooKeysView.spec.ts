...
import KeysView from '@/custom/user/WegooKeysView.vue'
  ...
  template: `
    <div>
      ...
      <div v-for="row in data" :key="row.id">
        ...
        <div
          v-if="columns.some((col) => col.key === 'current_concurrency')"
          data-test="current-concurrency"
        >
...
describe('user KeysView column settings', () => {
  ...
  it('keeps the last used IP column hidden in the column settings by default', async () => {
    const wrapper = await mountView()

    expect(wrapper.find('button[title="Column Settings"]').exists()).toBe(true)
    expect(visibleColumnKeys(wrapper)).not.toContain('last_used_ip')
    expect(visibleColumnKeys(wrapper)).not.toContain('current_concurrency')
  ...
  it('shows a hidden column when toggled and persists the preference', async () => {
    ...
    expect(localStorage.getItem('api-key-hidden-columns')).toBe(
      JSON.stringify(['current_concurrency', 'last_used_at', 'last_used_ip'])
    )
    expect(localStorage.getItem('api-key-column-settings-version')).toBe('3')
  })

  it('shows the last used IP column when saved column settings explicitly show it', async () => {
    localStorage.setItem(
      'api-key-hidden-columns',
      JSON.stringify(['key', 'rate_limit', 'expires_at', 'status', 'last_used_at', 'created_at'])
    )
  ...
  it('ignores legacy column preferences from localStorage', async () => {
    ...
    expect(visibleColumnKeys(wrapper)).not.toContain('last_used_ip')
    expect(localStorage.getItem('api-key-hidden-columns')).toBe(
      JSON.stringify(['group', 'created_at', 'last_used_ip', 'current_concurrency'])
    )
    expect(localStorage.getItem('api-key-column-settings-version')).toBe('3')
  })

  it('shows the last used IP column when toggled from column settings', async () => {
    listKeys.mockResolvedValueOnce({
      items: [{ ...createApiKey(), last_used_ip: '203.0.113.10' }],
      total: 1,
      page: 1,
  ...
  it('marks current concurrency as sortable', async () => {
    ...
    await wrapper.get('button[title="Column Settings"]').trigger('click')
    await getButtonByText(wrapper, 'Current concurrency').trigger('click')
    await nextTick()
