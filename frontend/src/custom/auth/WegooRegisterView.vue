<template>
  <AuthLayout>
    <div class="space-y-5 sm:space-y-6">
      ...
      <div
        v-if="contactInfo"
        class="rounded-lg border border-primary-100 bg-primary-50/80 px-3 py-3 text-sm leading-5 dark:border-primary-500/30 dark:bg-primary-500/10"
      >
        <div class="flex items-start gap-2.5">
          <Icon name="chatBubble" size="sm" class="mt-0.5 shrink-0 text-primary-600 dark:text-primary-300" />
          <div class="min-w-0">
            <p class="font-medium text-primary-800 dark:text-primary-200">
              {{ t('auth.registrationSupportTitle') }}
            </p>
            <p class="mt-1 break-words text-primary-700 dark:text-primary-300">
              {{ t('auth.registrationSupportDesc', { contact: contactInfo }) }}
            </p>
          </div>
        </div>
      </div>
...
import WechatOAuthSection from '@/custom/auth/WegooWechatOAuthSection.vue'
import EmailOAuthButtons from '@/components/auth/EmailOAuthButtons.vue'
import LoginAgreementPrompt from '@/custom/auth/WegooLoginAgreementPrompt.vue'
