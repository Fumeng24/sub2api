<template>
  <div class="gateway-callback-shell dark min-h-screen px-4 py-10 text-white">
    <div class="mx-auto max-w-2xl">
      <div v-if="isProcessing" data-testid="oauth-callback-processing" class="gateway-callback-card p-6 text-center">
        <div class="mx-auto h-8 w-8 animate-spin rounded-full border-2 border-[#4dd4e6] border-t-transparent"></div>
        <h1 class="mt-4 text-lg font-semibold text-white">
      ...
      <div v-else-if="needsRegistrationCompletion" data-testid="oauth-callback-registration" class="gateway-callback-card p-6">
        ...
        <div class="mt-6 space-y-4">
          ...
          <p v-if="registrationError" class="rounded-lg border border-[#e8a87c]/30 bg-[#e8a87c]/10 px-3 py-2 text-sm text-[#f0c2a7]">
            {{ registrationError }}
          </p>
          <button
      ...
      <div v-else-if="invalidCallback" data-testid="oauth-callback-invalid" class="gateway-callback-card p-6 text-center">
        <h1 class="text-lg font-semibold text-white">
      ...
      <div v-else data-testid="oauth-callback-manual" class="gateway-callback-card p-6">
        <h1 class="text-lg font-semibold text-white">
          {{ t('auth.oauth.callbackTitle') }}
        </h1>
        <p class="mt-2 text-sm text-white/58">
...
.gateway-callback-shell :deep(.input) {
  min-height: 2.75rem;
...
.gateway-callback-shell :deep(.input-label) {
...
.gateway-callback-shell :deep(.btn-secondary) {
  border-color: rgba(255, 255, 255, 0.10);
  background: rgba(255, 255, 255, 0.045);
