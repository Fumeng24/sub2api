...
const defaultLoginAgreementUpdatedAt = "2026-06-12";
const defaultTermsContentMd = `# 服务条款

欢迎使用本站服务。你在注册、登录、购买、创建 API 密钥或使用本站 API 时，即表示你已阅读、理解并同意本条款、使用政策、支持的国家与地区说明以及服务特定条款。

## 服务范围

本站提供 AI API 聚合、模型调用、用量统计、余额结算、密钥管理等相关服务。本站仅面向符合服务地区要求的用户提供服务。

## 地区限制

本站仅面向符合服务地区要求的用户提供服务。受限制地区或依法、依规、依平台规则不得使用本服务的人员、组织或实体，不得注册、登录、购买、访问或调用本站服务。

如果你属于受限制对象，或代表受限制地区的个人、组织、实体使用本站服务，请立即停止注册、登录、购买和调用 API。你继续使用本站服务，即视为你确认自己不属于上述限制对象，并承诺你的使用行为符合所在地法律法规及本站规则。
...
本站按页面展示的计费规则、分组倍率、模型价格和实际用量扣费。模型价格、分组倍率、可用模型、可用功能可能因上游能力、成本、风控或运营策略变化而调整。调整后继续使用服务，视为你接受调整后的规则。

## 服务变更与中断

本站会尽力保障服务稳定，但不承诺服务永不中断或所有模型始终可用。因上游服务异常、网络故障、维护升级、风控策略、不可抗力或第三方原因导致的不可用、延迟、失败、降级或功能变化，本站可根据实际情况处理。

## 禁止行为

你不得滥用、攻击、干扰本站服务，不得绕过限额、风控、计费或访问控制，不得批量注册、倒卖账号、转售本站服务、盗用他人身份或凭据，不得使用本站从事违法违规、侵权、欺诈、骚扰、恶意自动化或损害第三方权益的行为。

## 内容责任

你对输入内容、生成内容、使用场景及后续传播承担责任。AI 生成内容可能不准确、不完整或不适合特定用途，你应自行判断、核验并承担使用风险。

## AI 生成内容标识

本站提供的文本、图片、音频、视频、文件或其他生成合成内容，可能包含显式提示、元数据、水印或其他形式的 AI 生成标识。你不得删除、篡改、隐藏、伪造或规避相关标识，也不得诱导他人误认 AI 生成内容为人工原创、真实事件、真实证据或未经处理的客观记录。

## 对外发布责任

你通过本站生成、加工、下载、复制、转发、发布或分发内容时，应自行确保相关内容符合法律法规、平台规则、第三方权利和本站政策要求。涉及对外展示、传播、商业使用、媒体发布或可能影响他人权益的内容，你应按适用规则进行必要标识、审核和说明。

## 日志与风控

为保障计费准确、服务稳定、安全风控、争议处理和合规审计，本站可能记录账号、API 密钥、请求时间、请求 ID、模型、分组、用量、余额变动、网络信息、错误信息和风控处置等必要运行日志。本站可基于异常调用、违规风险、投诉争议或安全事件，对相关账号、API 密钥、请求能力或余额使用采取限制措施。

## 违规处理

如果你违反本条款或本站其他规则，本站有权限制、暂停或终止你的账号、API 密钥、余额使用、购买能力或相关服务，并保留追究责任的权利。

## 条款更新

本站可根据业务、合规、上游规则或安全需要更新条款。条款更新后，可能要求你重新确认。你继续使用本站服务，即视为接受更新后的条款。`;
const defaultUsagePolicyContentMd = `# 使用政策

本政策用于说明本站允许和禁止的使用方式。你使用本站服务时，应同时遵守服务条款、支持的国家与地区说明、服务特定条款以及适用法律法规。

## 基本要求

你应以合法、合规、诚信、安全的方式使用本站服务，不得将本站用于规避监管、绕过限制、攻击系统、侵犯他人权益或制造现实伤害。
...
- 以转售、共享、出租、批发、镜像或其他方式向第三方提供本站账号、余额、API 密钥或服务能力，除非获得本站明确书面许可。
- 使用受限制地区人员身份、网络环境、支付方式或组织关系注册、购买、访问或调用本站服务。
...
为保障服务安全和稳定，本站可对异常请求、违规内容、异常账号、异常密钥或异常支付行为进行监测、限制、暂停、封禁、删除或要求补充说明。

## 用户责任

你对通过本站提交的输入、上传的文件、调用的模型、生成的输出以及后续使用方式承担全部责任。你应确保相关行为符合法律法规、第三方平台规则和本站政策。`;
const defaultSupportedRegionsContentMd = `# 支持的国家与地区

本站仅向下列服务可用国家与地区，且当地法律法规允许使用本站服务的用户提供服务。未列入本页的国家或地区，默认不在当前服务范围内。
...
如果你属于以下任一情况，请勿注册、登录、购买或使用本站服务：

- 你所在的国家或地区未列入当前支持列表。
- 你是受限制地区人员，或代表受限制地区个人、组织、实体使用本站服务。
- 你的使用行为会导致本站、你本人或任何第三方违反适用法律法规、监管要求、制裁规则、平台规则或合同义务。
- 你试图通过代理、虚拟身份、代付、账号共享或其他方式规避本站服务地区限制。

## 可用性说明

支持列表不代表所有模型、支付方式、网络环境或功能在当地均可使用。实际可用性可能受到支付渠道、网络环境、上游服务政策、风控系统和具体模型能力影响。

## 用户声明

你注册、登录、购买或使用本站服务，即表示你声明并保证：

- 你所在的国家或地区已列入当前支持列表。
- 你不是受限制地区人员，也不是代表受限制地区个人、组织或实体使用本站服务。
- 你使用本站服务的所在地允许你访问和使用相关 AI、API、支付和网络服务。
- 你不会通过技术或身份手段规避本站地区限制。

## 处置措施

如果本站发现或有合理理由认为你的账号、支付、网络、身份、组织关系或使用行为不符合服务地区要求，本站有权限制、暂停或终止你的账号、API 密钥、余额使用和相关服务。`;
const defaultServiceSpecificTermsContentMd = `# 服务特定条款

本条款适用于本站提供的 AI API、模型调用、图像生成、余额计费、分组倍率、渠道调度、API 密钥和相关增值功能。

## AI 模型服务

本站提供的模型能力可能来自不同模型类型、接口形式或服务能力。模型名称、上下文长度、图像能力、工具能力、速率限制、响应格式和可用性可能随时变化。页面展示的信息仅代表当前配置或最近同步结果，实际调用结果以服务运行时状态为准。

## 调度与可用性

本站会通过调度策略提升整体稳定性，但不保证任一具体请求、模型、分组或功能必定成功。上游异常、网络波动、限流、余额不足、模型下线、策略调整或风控限制都可能导致请求失败、重试、降级或暂时不可用。

## 余额、倍率与扣费

本站余额为站内计费单位。不同分组、模型、功能和请求类型可能适用不同倍率或计费方式。图像生成、缓存、长上下文、工具调用、重试、失败响应等场景的计费规则，以页面展示、后台配置和实际账单记录为准。
...
AI 输出可能包含错误、遗漏、过时信息、幻觉或不适合实际使用的内容。你应自行核验输出，不得在没有人工复核的情况下将输出用于高风险决策或直接影响他人权益的场景。

## 服务调整

本站可根据成本、稳定性、合规、安全、上游政策或运营需要调整模型、分组、倍率、功能、限额、计费规则、可用地区和访问方式。对于明显异常、滥用、违规或高风险使用，本站可采取限制、暂停、终止或其他必要措施。`;
