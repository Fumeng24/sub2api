  ...
  <AppLayout>
    ...
    >
      <div v-if="selectedTicket" class="space-y-5">
        ...
        >
          <div class="grid grid-cols-1 gap-3 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,2fr)_auto]">
            <div>
              <label class="input-label">{{ t('admin.tickets.balance.operation') }}</label>
              <Select v-model="balanceForm.operation" :options="balanceOperationOptions" />
            </div>
            <div>
...
import { useAppStore, useAuthStore, useTicketStore } from '@/stores'
...
async function fetchTickets() {
  loading.value = true
  try {
    const res = await adminAPI.tickets.list(pagination.page, pagination.page_size, buildListFilters())
...
async function fetchStats() {
  try {
    ticketStats.value = await adminAPI.tickets.getStats()
...
async function loadTicketDetail(id: number) {
  ...
  try {
    const detail = await adminAPI.tickets.getById(id)
...
async function handleClaimTicket() {
  ...
  claiming.value = true
  try {
    const updated = await adminAPI.tickets.claim(selectedTicket.value.id)
    selectedTicket.value = updated
    syncTicketForm(updated)
...
async function handleEscalateTicket() {
  ...
  try {
    const updated = await adminAPI.tickets.escalate(selectedTicket.value.id, reason.trim())
    selectedTicket.value = updated
    syncTicketForm(updated)
...
async function handleBalanceAdjust() {
  ...
  try {
    await adminAPI.tickets.adjustBalance(selectedTicket.value.id, {
...
async function handleAutoClose() {
  ...
  try {
    const res = await adminAPI.tickets.autoCloseResolved(autoCloseDays.value)
