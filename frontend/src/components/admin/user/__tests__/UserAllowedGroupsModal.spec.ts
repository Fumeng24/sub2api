...
describe('UserAllowedGroupsModal', () => {
  it('saves batch discount as dynamic group_discounts instead of fixed rates', async () => {
    const wrapper = await mountAndOpen()

    await wrapper.get('[data-testid="bulk-discount-input"]').setValue('8')
    await wrapper.get('[data-testid="apply-bulk-discount"]').trigger('click')
