...
describe('UseKeyModal', () => {
  it('shows the selected group, platform, and all configured endpoints', () => {
        ...
        customEndpoints: [
          {
            ...
            description: 'Low latency for Asia',
          },
        ],
      },
      global: {
    ...
    expect(wrapper.get('[data-testid="use-key-group-context"]').text()).toContain('Codex Pro')
    expect(wrapper.get('[data-testid="use-key-platform-context"]').text()).toContain('OpenAI')
    const endpoints = wrapper.get('[data-testid="use-key-endpoint-context"]').text()
    expect(endpoints).toContain('https://api.example.com/v1')
    expect(endpoints).toContain('Hong Kong Relay')
    expect(endpoints).toContain('https://hk.example.com/v1')
  })
